...
public enum Complicacao implements Checavel, Identificavel{
	...
	private final TipoComplicacao[] tipoPossivel;
	...
	public static Complicacao getAleatoria() {
		...
		random = (int) (Math.random() * comp.getTipoPossivel().length);
		comp.setTipo(comp.getTipoPossivel()[random]);
	...
	public TipoComplicacao[] getTipoPossivel() {
		return tipoPossivel;
	}
	...
	public void setTipo(TipoComplicacao tipo) {
		...
		for(TipoComplicacao x: tipoPossivel) {
			if(x.equals(tipo)) {
				tipoValido = true;
				break;
			}
